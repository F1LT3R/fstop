...
export class GitStatus {
	...
	/**
	 * Get GitStatus for a path using real path (for symlinks)
	 * Uses cached data, so must be called after getForPath has been called for that path
	 * @param {string} path - Path to look up (can be symlink path)
	 * @param {string} realPath - Real path if symlink, null otherwise
	 * @param {string} type - 'file' or 'directory'
	 * @returns {object|null} Status object or null
	 */
	static getStatusForPath(path, realPath, type) {
		const pathForGit = realPath || path
		
		// Find the MOST SPECIFIC (deepest/longest) git root for this path
		// This ensures nested repos (like symlinked dirs) take precedence
		let bestMatchGitStatus = null
		let bestMatchLength = 0
		
		for (const [gitRoot, gitStatus] of gitStatusCache.entries()) {
			// Check if pathForGit is under this git root
			if (pathForGit === gitRoot || pathForGit.startsWith(gitRoot + '/')) {
				// Use the longest matching git root (most specific)
		...
		const gitRoot = bestMatchGitStatus.gitRoot
		if (gitRoot) {
			pathToRootCache.set(pathForGit, gitRoot)
		}
		
		return bestMatchGitStatus.getStatus(pathForGit, type)
	}
