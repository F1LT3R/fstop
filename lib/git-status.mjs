...
export class GitStatus {
	constructor(rootPath) {
		...
		this.ignoredFiles = new Set() // full paths of gitignored files
		this.ignoredDirs = new Set()  // full paths of gitignored directories
		this.ahead = 0
		this.behind = 0
	...
	/**
	 * Check if a path is gitignored (static method)
	 * Finds the best-match GitStatus instance and delegates to isIgnored()
	 * @param {string} path - Path to check
	 * @param {string} type - 'file' or 'directory'
	 * @returns {boolean} True if gitignored
	 */
	static isPathIgnored(path, type) {
		let bestMatch = null
		let bestLen = 0

		for (const [gitRoot, gitStatus] of gitStatusCache.entries()) {
			if (path === gitRoot || path.startsWith(gitRoot + '/')) {
				if (gitRoot.length > bestLen) {
					bestMatch = gitStatus
					bestLen = gitRoot.length
				}
			}
		}

		if (!bestMatch) return false
		return bestMatch.isIgnored(path, type)
	}
	...
	async refresh(force = false) {
		...
		// Check ignored set changes
		const newIgnoredSize = newIgnored.files.size + newIgnored.dirs.size
		if (newIgnoredSize !== oldIgnoredSize) {
			changed = true
		}

		// Atomically swap in the new data
		this.fileStatus = newFileStatus
		this.ignoredFiles = newIgnored.files
		this.ignoredDirs = newIgnored.dirs
	...
	/**
	 * Fetch gitignored files and directories into provided sets
	 * Uses --directory flag to get ignored dir names without recursing into them
	 * @param {object} target - { files: Set, dirs: Set } to populate
	 */
	async fetchIgnoredFilesInto(target) {
		try {
			const { stdout } = await execAsync(
			...
			for (const line of stdout.split('\n')) {
				...
				if (line.endsWith('/')) {
					// Directory (strip trailing slash)
					target.dirs.add(this.rootPath + '/' + line.slice(0, -1))
				} else {
					target.files.add(this.rootPath + '/' + line)
				}
			}
		} catch {
	...
	isIgnored(path, type) {
		...
		if (type === 'file') {
			return this.ignoredFiles.has(path)
		}
		return this.ignoredDirs.has(path)
	}
