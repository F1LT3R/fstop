...
export class TreeState {
	...
	setNode(fullPath, type, eventType) {
		...
		// Mark as touched when a real event occurs (not initial tree building)
		if (eventType) {
	...
	propagateToParents(fullPath, eventType, now) {
		...
		while (parentPath.length >= this.rootPath.length) {
			...
			if (parentNode) {
				// Mark parent as touched when a real event occurs
				if (eventType) {
					parentNode.wasTouched = true
				}
	 ...
	 * Git-tracked deletions stay visible while git shows them as deleted
	 * Untracked deletions are removed when heat drops below threshold
	 * @param {object} gitStatus - GitStatus instance for checking tracked files
	 */
	advanceGhosts(gitStatus = null) {
		let removed = false
		
		for (const [path, ghost] of this.ghosts) {
			const hasGitStatus = gitStatus?.getStatus(path, ghost.node.type)
			
			if (hasGitStatus) {
				// Git-tracked deletion - keep visible while git knows about it
				continue
			}
			
			// Untracked - remove when heat drops below threshold
			if (!isHot(ghost.node.heat)) {
	...
	/**
	 * Immediately remove all ghost nodes from the tree
	 */
	clearGhosts() {
		for (const [path] of this.ghosts) {
			this.fullyRemoveNode(path)
		}
		this.ghosts.clear()
	}
	...
	calculateNodeHeat(node, now) {
		...
		// Note: Ghost heat boost removed - ghosts now decay naturally
		// Git-tracked deletions stay visible via advanceGhosts() git check
		// Untracked deletions fade based on normal heat decay (~10s)
	...
	/**
	 * Check if any nodes have heat > 0 (still cooling down)
	 * Used to optimize render cycles - skip redraws when nothing is hot
	 * Uses isHot() threshold (20) instead of heat > 0 to stop redraws sooner
	 * @returns {boolean} True if any items are still hot
	 */
	hasHotItems() {
		for (const node of this.nodes.values()) {
			if (isHot(node.heat)) return true
		}
		return false
	}
	...
	/**
	 * Detect and mark symlinked directories (one-time operation)
	 * Uses async operations with Promise.all() for parallel performance
	 * Only checks directories, not files
	 */
	async detectSymlinks() {
		// First pass: detect symlinks in parallel
		const checks = []
		for (const node of this.nodes.values()) {
			if (node.type === 'directory') {
				checks.push(
					lstat(node.path)
						.then(stats => {
							if (stats.isSymbolicLink()) {
								return realpath(node.path).then(real => {
									node.isSymlink = true
									node.realPath = real
								})
							}
		...
		// Second pass: compute real paths for children of symlinked directories
		for (const node of this.nodes.values()) {
			if (!node.isSymlink && !node.realPath) {
				// Check if this node is under a symlinked directory
				const realPath = this.computeRealPath(node.path)
				if (realPath !== node.path) {
	...
	/**
	 * Compute real path for a node by checking if any parent is a symlink
	 * @param {string} path - Path to resolve
	 * @returns {string} Real path (same as input if no symlink parents)
	 */
	computeRealPath(path) {
		// Find if any parent directory is a symlink
		let checkPath = dirname(path)
		while (checkPath.length >= this.rootPath.length) {
			const parentNode = this.nodes.get(checkPath)
			if (parentNode && parentNode.isSymlink && parentNode.realPath) {
				// Found a symlinked parent - replace the symlink part with real part
				const relative = path.substring(checkPath.length)
				return parentNode.realPath + relative
			}
			if (checkPath === this.rootPath) break
