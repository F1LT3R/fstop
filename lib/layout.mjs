...
export const WEIGHT = {
	...
	context: {
		...
		touched:             75,  // Modified this session
...
function shouldAutoCollapse(node, gitStatus) {
	// Never collapse if it has git status, heat, or was touched this session
	if (hasGitStatus(node, gitStatus) || isHot(node.heat || 0) || node.wasTouched) {
		return false
	}
 ...
 */
function flattenTree(treeNode, depth, isLast, parentContinues, treeState, gitStatus, collapse = true) {
	...
	if (treeNode.type === 'directory' && treeNode.children.size > 0) {
		...
		if (collapse && depth > 0 && gitStatus && shouldAutoCollapse(treeNode, gitStatus)) {
...
export function generateLayout(treeState, options = {}) {
	...
	const allLines = generateAllLines(treeState, gitStatus, filterPattern, rootPath, availableRows)
 ...
 */
function generateAllLines(treeState, gitStatus, filterPattern = '', rootPath = '', availableRows = Infinity) {
	// First pass: flatten without collapsing to see if everything fits
	const expandedNodes = flattenTree(treeState.root, 0, true, [], treeState, gitStatus, false)

	// Only collapse if the expanded tree exceeds available space
	const allNodes = expandedNodes.length > availableRows
		? flattenTree(treeState.root, 0, true, [], treeState, gitStatus, true)
		: expandedNodes
