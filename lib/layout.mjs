 ...
 * @param {object} node - Tree node to count
 * @returns {number}
 */
...
function flattenTree(treeNode, depth, isLast, parentContinues, treeState, gitStatus) {
	...
	if (treeNode.type === 'directory' && treeNode.children.size > 0) {
		// Check if this directory should be auto-collapsed
		if (gitStatus && shouldAutoCollapse(treeNode, gitStatus)) {
			layoutNode.collapsed = true
			layoutNode.childCount = countAllDescendants(treeNode)
