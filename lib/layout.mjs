...
import { isHot, compareByHeatAsc, HEAT_CONFIG } from './heat.mjs'
...
export const LAYOUT_CONFIG = {
	...
	minRows: 5,
	// Extra detail lines per hot file when space is abundant
	maxDetailLines: 3,
	// Threshold for "abundant space" (ratio of available to needed)
	abundantSpaceRatio: 1.5,
}
...
export function generateLayout(treeState, options = {}) {
	...
	if (rowsNeeded <= availableRows) {
		// We have enough space - possibly expand hot items with detail
		const result = expandHotItems(allNodes, availableRows, rowsNeeded, now)
		return {
			nodes: result.nodes,
...
/**
 * Expand hot items with extra detail when space is abundant
 * @param {object[]} nodes - Layout nodes
 * @param {number} availableRows - Available rows
 * @param {number} currentRows - Current row count
 * @param {number} now - Current timestamp
 * @returns {object} Result with nodes and total rows
 */
function expandHotItems(nodes, availableRows, currentRows, now) {
	const extraRows = availableRows - currentRows
	
	if (extraRows <= 0) {
		return { nodes, totalRows: currentRows }
	}
	
	// Find hot files that could use extra detail
	const hotFiles = nodes.filter(n => n.type === 'file' && isHot(n.heat))
	
	if (hotFiles.length === 0) {
		return { nodes, totalRows: currentRows }
	}
	
	// Distribute extra rows among hot files (hottest get more)
	hotFiles.sort((a, b) => b.heat - a.heat)
	
	let remaining = extraRows
	for (const node of hotFiles) {
		if (remaining <= 0) break
		
		// Give up to maxDetailLines extra lines to each hot file
		const linesToAdd = Math.min(
			LAYOUT_CONFIG.maxDetailLines,
			Math.ceil(remaining / hotFiles.length)
		)
		node.detailLines = linesToAdd
		remaining -= linesToAdd
	}
	
	const totalRows = nodes.reduce((sum, n) => sum + 1 + n.detailLines, 0)
