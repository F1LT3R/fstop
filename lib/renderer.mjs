...
function renderNodeLine(node, now, maxWidth, gitStatus = null, filterMatch = false, filterPattern = '') {
	...
	// Add symlink indicator AFTER hyperlink (so it's not underlined)
	if (node.type === 'directory' && node.isSymlink) {
		leftPart += chalk.magenta(`@`)
		leftLen += 1  // Add 1 for the @ character
	}
	
	// Add collapsed indicator AFTER hyperlink (so it's not underlined)
	if (node.collapsed) {
		leftPart += chalk.gray(`…`)
		leftLen += 1  // Add 1 for the … character
	}
...
function renderWithInteractive(layout, watchPath, gitStatus = null, interactive = {}, lastFrameLines = []) {
	const { cursorIndex = -1, filterPattern = '', quick = false } = interactive
	const now = Date.now()
	const { rows, cols } = layout.terminalSize
