...
import { GIT_COLORS, GitStatus } from './git-status.mjs'
...
const RENDER_CONFIG = {
	...
	updateChar: '●',
	// Cursor styling
	cursorIndicator: '<',  // ASCII-safe for monochrome/text files
	cursorBg: 'bgRed',     // chalk bg method: bgRed, bgBlue, bgMagenta, etc.
}
 ...
 * @param {boolean} mdPreview - Whether markdown preview mode is active
 * @returns {string} Rendered line
 */
function renderNodeLine(node, now, maxWidth, gitStatus = null, filterMatch = false, filterPattern = '', mdPreview = false) {
	const isMdFile = mdPreview && node.name.toLowerCase().endsWith('.md')
	const isIgnored = GitStatus.isPathIgnored(node.path, node.type)
	let prefix = renderTreePrefix(node)

	// Color tree branch magenta for .md files in preview mode
	if (isMdFile) {
		prefix = chalk.magenta(prefix)
	} else if (isIgnored) {
	...
	// Build name with type indicator (but NOT symlink @ or collapsed indicator yet)
	let displayName = node.name
	if (node.type === 'directory') {
		displayName += '/'
	}

	// Git symbol length (needed for truncation calc)
	const gitSymbolLen = gitStatus ? 2 : 0  // symbol + space

	// Truncate name to fit available width (before any ANSI styling)
	const prefixLen = visibleLength(prefix)
	const maxNameLen = maxWidth - prefixLen - gitSymbolLen
	if (displayName.length > maxNameLen) {
		displayName = truncate(displayName, Math.max(1, maxNameLen))
	}

	// Apply filter highlight if filterMatch provided
	if (filterMatch && filterMatch.match) {
		displayName = highlightMatch(displayName, filterPattern)
	}
	...
	let leftLen = visibleLength(prefix) + gitSymbolLen + displayName.length
	...
	} else if (hasGitStatus || isHot(node.heat)) {
		coloredName = colorFn.bold(styledName)
	} else if (isIgnored) {
		coloredName = chalk.dim(styledName)
	} else {
		coloredName = chalk.white(styledName)
	}

	if (mdPreview && isMdFile) {
		// markdownRenderer mode: magenta underline, mouse tracking handles clicks
		coloredName = '\x1b[4m\x1b[58;5;5m' + coloredName + '\x1b[59m\x1b[24m'
		leftPart += coloredName
	} else {
		// OSC8 mode: terminal-native clickable hyperlink with grey underline
		const fileUrl = 'file://' + node.path
		const greyUnderlined = '\x1b[4m\x1b[58;5;245m' + coloredName + '\x1b[59m\x1b[24m'
		leftPart += hyperlink(greyUnderlined, fileUrl)
	}

	// Add symlink indicator after name
	if (node.type === 'directory' && node.isSymlink) {
		leftPart += chalk.magenta(`@`)
		leftLen += 1  // Add 1 for the @ character
	}
	
	// Add collapsed indicator after name
	if (node.collapsed) {
		leftPart += chalk.gray(`…`)
		leftLen += 1  // Add 1 for the … character
	}

	// Directory fold/unfold toggle indicator
	if (node.type === 'directory' && node.depth > 0) {
		if (node.collapsed || node.manualFold) {
			leftPart += ' ' + chalk.yellow('[▶▶▶]')
			leftLen += 6  // space + 5 chars
		} else {
			leftPart += ' ' + chalk.cyan('[▼▼▼]')
			leftLen += 6
		}
	...
	// Combine parts — always pad with non-linked spaces after the name
	// to prevent terminals from extending the OSC8 hyperlink clickable area to the edge
	if (rightPart) {
		return leftPart + ' '.repeat(padding) + rightPart
	}
	const trailingSpace = maxWidth - leftLen
	if (trailingSpace > 0) {
		return leftPart + ' '.repeat(trailingSpace)
	}
	return leftPart
}
...
export function render(layout, watchPath, gitStatus = null) {
	...
	for (const line of linesToRender) {
		if (line.lineType === 'node') {
			...
			const nodeGitStatus = gitStatus ? GitStatus.getStatusForPath(node.path, node.realPath, node.type) : null
...
/**
 * Highlight matched portion of name with dim yellow background
 * Simple text-only matching
 * @param {string} name - The name to highlight
 * @param {string} pattern - Filter pattern
 * @returns {string} Name with highlighted match
 */
function highlightMatch(name, pattern) {
	if (!pattern) return name
	
	// Clean pattern - remove leading/trailing slashes
 ...
 * @param {string} watchPath - Path being watched
 * @param {object} gitStatus - GitStatus instance (optional)
 * @param {object} interactive - { cursorIndex, filterPattern, quick }
 * @param {string[]} lastFrameLines - Previous frame buffer for diffing (modified in place)
 */
function renderWithInteractive(layout, watchPath, gitStatus = null, interactive = {}, lastFrameLines = []) {
	const { cursorIndex = -1, filterPattern = '', quick = false, mdPreview = false } = interactive
	const now = Date.now()
	const { rows, cols } = layout.terminalSize
	
	// Build new frame in memory first
	const newFrameLines = []
	
	// Build header
	const headerLines = renderHeader(watchPath, layout.terminalSize, gitStatus)
	headerLines.forEach((line) => {
		newFrameLines.push(line)
	})
	...
	for (let i = 0; i < linesToRender.length; i++) {
		...
		if (line.lineType === 'node') {
			...
			const nodeGitStatus = gitStatus ? GitStatus.getStatusForPath(node.path, node.realPath, node.type) : null
			const filterMatch = line.filterMatch || false
			let renderedLine = renderNodeLine(node, now, isCursor ? cols - 1 : cols, nodeGitStatus, filterMatch, filterPattern, mdPreview)

			// Apply cursor styling - background with indicator on last column
			if (isCursor) {
				renderedLine = chalk[RENDER_CONFIG.cursorBg](renderedLine + RENDER_CONFIG.cursorIndicator)
			}
			
			newFrameLines.push(renderedLine)
		}
		currentRow++
	}
	
	// In quick mode, skip blank lines and filter input
	if (!quick) {
		// Add blank lines (always leave room for filter input)
		const footerStart = rows - LAYOUT_CONFIG.footerRows - 1
		while (currentRow <= footerStart) {
			newFrameLines.push('')
			currentRow++
		}
		
		// Always render filter input (always listening)
		const filterLine = chalk.cyan.bold('/') + filterPattern + chalk.gray('_')
		newFrameLines.push(filterLine)
		...
		// Quick mode: render footer immediately after content
		const footerLines = renderFooter(layout, gitStatus)
		footerLines.forEach((line) => {
			newFrameLines.push(line)
		})
	...
	// Compare with last frame and only write changed lines
	const isFirstRender = lastFrameLines.length === 0
	
	if (isFirstRender) {
		// First render or after resize - clear screen and write everything
		clearScreen()
		newFrameLines.forEach((line, i) => {
			writeLine(i + 1, line)
		})
	} else {
		// Incremental update - only write changed lines
		const maxLines = Math.max(newFrameLines.length, lastFrameLines.length)
		for (let i = 0; i < maxLines; i++) {
			const newLine = newFrameLines[i] || ''
			const oldLine = lastFrameLines[i] || ''
			
			if (newLine !== oldLine) {
				writeLine(i + 1, newLine)
			}
		}
	}
	
	// Update last frame buffer (modify in place to preserve reference)
	lastFrameLines.length = 0
	lastFrameLines.push(...newFrameLines)
	
	// In quick mode, add trailing newline
	if (quick) {
...
export function createRenderer(watchPath) {
	...
	let lastFrameLines = []  // Frame buffer for diffing
	let lastTerminalSize = null  // Track terminal size for invalidation
	...
	return {
		 ...
		 * @param {object} interactive - Interactive state { cursorIndex, filterPattern }
		 */
		render(layout, gitStatus = null, interactive = {}) {
			lastLayout = layout
			
			// Check if terminal was resized - invalidate frame buffer
			const currentSize = layout.terminalSize
			if (!lastTerminalSize || 
			    lastTerminalSize.rows !== currentSize.rows || 
			    lastTerminalSize.cols !== currentSize.cols) {
				lastFrameLines = []  // Force full repaint
				lastTerminalSize = { ...currentSize }
			}
			
			renderWithInteractive(
				layout, 
				watchPath, 
