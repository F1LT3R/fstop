...
function renderNodeLine(node, now, maxWidth, gitStatus = null) {
	...
	// Add git status symbol before name
	const gitSymbol = renderGitSymbol(gitStatus)
	line += gitSymbol
	
	// Apply color to name (git status color or heat color)
	...
	if (showHeatIndicators) {
		...
		if (timeAgo) {
			line += ' ' + style(timeAgo, ANSI.fg.gray)
		}
		...
		line += ' ' + style(`${heatPct}%`, ANSI.fg.gray, ANSI.dim)
	} else if (node.type === 'directory' && node.changeCount > 0 && node.depth > 0) {
		// Show change count for directories with activity (not root)
...
/**
 * Build tree continuation prefix for detail lines
 * @param {object} node - Layout node
 * @returns {string} Tree continuation prefix
 */
function buildDetailPrefix(node) {
	let prefix = ''
	
	// Add vertical lines for parent levels (same as node)
	for (let i = 0; i < node.parentContinues.length; i++) {
		if (node.parentContinues[i]) {
			prefix += BOX.vertical + '   '
		} else {
	...
	// Add continuation for the node's own level
	// If node is not last, there's a sibling below, so continue the line
	// Detail lines are "inside" the node, so we indent further
	if (node.depth > 0) {
		prefix += (node.isLast ? ' ' : BOX.vertical) + '   '
	}
	
	// Extra indent to show detail belongs to the file
	prefix += '    '
...
function renderDetailLines(node, now) {
	...
	const lines = []
	const prefix = buildDetailPrefix(node)
	...
	if (node.detailLines >= 1 && node.eventType) {
		const eventLabel = formatEventType(node.eventType)
		const timeAgo = formatTimeAgo(node.eventTime, now)
		lines.push(prefix + style(`├─ ${eventLabel} ${timeAgo} ago`, ANSI.fg.gray, ANSI.dim))
	}
	...
	if (node.detailLines >= 2) {
		const bar = heatBar(node.heat)
		const heatPct = Math.round(node.heat)
		lines.push(prefix + style(`├─ heat: ${bar} ${heatPct}%`, ANSI.fg.gray, ANSI.dim))
	}
	
	// Detail line 3: Path info
	if (node.detailLines >= 3) {
		lines.push(prefix + style(`└─ ${node.path}`, ANSI.fg.gray, ANSI.dim))
	}
