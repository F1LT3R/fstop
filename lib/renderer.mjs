...
function renderWithInteractive(layout, watchPath, gitStatus = null, interactive = {}) {
	...
	if (!quick) {
		// Clear remaining lines (always leave room for filter input)
		...
		// Quick mode: render footer immediately after content, then add trailing newline
		const footerLines = renderFooter(layout, gitStatus)
		footerLines.forEach((line, i) => {
			writeLine(currentRow + i, line)
		})
		// Add trailing newline for clean terminal output
